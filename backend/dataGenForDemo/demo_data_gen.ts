...
const quizArray = [quizNamesFineArts, quizNameManagement];

const quizType = ["live", "self paced"];


const questionsPainting =
    [
        {question: "The Birth of Venus is considered to be one of the top 10 paintings of all time. Who made it?",
            imgUrl:"https://upload.wikimedia.org/wikipedia/commons/thumb/7/7d/Botticelli_Venus.jpg/512px-Botticelli_Venus.jpg",
        ...
        {question: "Which famous painting is also referred to as The Dutch Monalisa",
            imgUrl:"https://upload.wikimedia.org/wikipedia/commons/thumb/c/ce/Girl_with_a_Pearl_Earring.jpg/512px-Girl_with_a_Pearl_Earring.jpg",
        ...
        {question: "Guernica is one of Pablo Piccaso's famous paintings. In which year did he complete it?",
            imgUrl:"https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Mural_del_Gernika.jpg/512px-Mural_del_Gernika.jpg",
        ...
        {question: "Where would you find the painting The Last Supper",
            imgUrl:"https://upload.wikimedia.org/wikipedia/commons/thumb/0/08/Leonardo_da_Vinci_%281452-1519%29_-_The_Last_Supper_%281495-1498%29.jpg/512px-Leonardo_da_Vinci_%281452-1519%29_-_The_Last_Supper_%281495-1498%29.jpg",
            answers: [{correct: true, text: "Milan"}, {correct: false, text: "Venice"}, {correct: false, text: "Rome"}, {correct: false, text: "Naples"}]}
    ];
const questionsSculpture =
    [
        {question: "What can you infer about prehistoric sculpture?",
            imgUrl:"https://upload.wikimedia.org/wikipedia/commons/8/80/Huittisten_hirvenp%C3%A4%C3%A4.jpg",
        ...
        {question: "Which term best describes Michelangelo's sculpture of David?",
            imgUrl:"https://upload.wikimedia.org/wikipedia/commons/thumb/a/a0/%27David%27_by_Michelangelo_Fir_JBU002.jpg/320px-%27David%27_by_Michelangelo_Fir_JBU002.jpg",
        ...
        {question: "Tim's walking around a sculpture to get a good look at its back. What style of sculpture must it be?",
            imgUrl:"https://upload.wikimedia.org/wikipedia/commons/thumb/9/98/Wood_Bodhisattva.jpg/640px-Wood_Bodhisattva.jpg",
        ...
        {question: "Which of the following is a true statement about busts?",
            imgUrl:"https://upload.wikimedia.org/wikipedia/commons/thumb/d/d9/Bust_of_a_Man_by_the_studio_of_Francis_Harwood.jpg/360px-Bust_of_a_Man_by_the_studio_of_Francis_Harwood.jpg",
            answers: [{correct: true, text: "They never include legs"}, {correct: false, text: "They only depict famous people"},
                {correct: false, text: "They are the most common form of sculpture"}, {correct: false, text: "They cannot be made out of clay"}]
        },
    ];

const questionsForQuiz =  [questionsPainting, questionsSculpture];


const names = ["Haydon Holding",
    "Jamie Taylor",
    "Shawn Lee",
    "Leslie Guevara",
    "Cameron Bean",
...
export const accountDataGen = async () =>{
    ...
    for (let i = 0; i < names.length; i++) {
        const user = await authCntr.register({  email: (names[i].split(" ")[0]).toLowerCase() + email, password: password,  name: names[i] });
        const params = JSON.stringify({
            email: user.email,
            password: "12345678",
        });

       await Axios.post("http://localhost:3000/auth/login", params,{
        ...
        uploadProfilePicture(users[i].token, profilePictures[i]);

    };

    // Generating mgroups for two members
   await  generateGroupsForUsers([
        {user: users[0], groupNames: ["Fine Arts", "Architecture", "Art History", "Design", "Film Studies",  "Graphic Design"]},
        {user: users[1], groupNames: ["Management", "Accounting", "Entrepreneurship", "Finance","Marketing", "Negotiations"]}]);

    // Joining groups
    await joinGroups(users);

    // Generation of genuine quizes
    let questionArray = await questionDataGen(questionsPainting);
    await quizDataGen(users[0], users[0].groupsCreated[0].id, questionArray, quizNamesFineArts[0], quizType[0]);
    questionArray = await questionDataGen(questionsSculpture);
    await quizDataGen(users[0], users[0].groupsCreated[0].id, questionArray, quizNamesFineArts[1], quizType[1]);

    // Generation of dummy quizes for arts
    for (let i = 2; i< quizNamesFineArts.length; i++){
       await quizDataGen(users[0], users[0].groupsCreated[0].id, [], quizNamesFineArts[i], quizType[i%2]);
    }

    // Generation of dummy quizes for business
    for (let i = 0; i< quizNameManagement.length; i++){
       await quizDataGen(users[1], users[1].groupsCreated[0].id, [], quizNameManagement[i], quizType[i%2]);
    }
...
async function generateGroupsForUsers(usersAndGroupNames: any[]){
    for (let i = 0; i < usersAndGroupNames.length; i++) {
        for (let j = 0; j<usersAndGroupNames[i].groupNames.length; j++){
            const group = await groupCntr.createGroup(usersAndGroupNames[i].user.id, usersAndGroupNames[i].groupNames[j], false);
            usersAndGroupNames[i].user.groupsCreated.push(group);
        }
    }

    // Users join all groups that were noc created by themselves
}
...
async function joinGroups(users: any[]) {
    ...
    for (let i = 0; i < users.length; i++) {
        ...
        for (let e = 0; e < allGroupsNames.length; e++) {
            ...
            if (isOwner == false){
                const result = await groupCntr.joinGroup(userId, {name: groupName});
            }
...
async function quizDataGen(user: any, groupId: number[][], questions: TempQuestion[], quizInfo: any, quizType: string ) {
            ...
            for (let j = 0; j< quiz.questions.length; j++){
                uploadQuestionPicture(user, quiz.id, questions[j].imgUrl, quiz.questions[j].id);
            }
        }
};
